...
const ItemAnswer = ({ name, answer, isTrue, id }) => {
    ...
    return (
        <TouchableOpacity key={id} onPress={pressed}
            style={[styles.answerStyle,
            {
                backgroundColor: isCorrect === 1 ? 'green' : isCorrect === 2 ? 'red' : 'white'
            }]} >
...
export default function TestDetailScreen({ navigation: { goBack } }) {
    const renderItemAnswer = ({ item }) => (
        <ItemAnswer name={item.name} answer={item.answer} isTrue={item.isTrue} id={item.id} />
    );
